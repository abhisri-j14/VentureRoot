    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5 md:p-6 flex flex-col gap-6 h-full">
      ...
      <div className="flex gap-4">
        <div className="flex-1 bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col gap-1">
          ...
          <div className="text-4xl font-heading font-black text-gray-900">₹{data.observedMarketPrice}</div>
          <span className="text-sm text-gray-600 font-medium">per litre</span>
        </div>
        <div className="flex-1 bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex flex-col gap-1">
          ...
          <div className="text-4xl font-heading font-black text-emerald-700">₹{data.expectedLocalPrice}</div>
          <span className="text-sm font-bold text-emerald-600">{data.marketValue} Value</span>
        </div>
      </div>
      ...
      {data.priceRange && (
        <div className="flex flex-col gap-2">
          ...
          <div className="flex justify-between items-center">
            <span className="text-sm font-bold text-slate-500">₹{data.priceRange.min} min</span>
            <span className="text-xs text-slate-400 font-medium">Typical local market variance</span>
            <span className="text-sm font-bold text-slate-500">₹{data.priceRange.max} max</span>
          </div>
        </div>
      )}
      ...
      {data.pricingFactors && data.pricingFactors.length > 0 && (
        <div>
          <h4 className="text-base font-bold text-gray-900 mb-3">Key Pricing Factors</h4>
      ...
      {data.observations && data.observations.length > 0 && (
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
          <h4 className="text-base font-bold text-gray-900 mb-3">Key Observations</h4>
          <ul className="flex flex-col gap-2.5">
            {data.observations.map((obs, idx) => (
              <li key={idx} className="flex items-start gap-2 text-base text-gray-800 font-medium">
